...
class action(Resource):
    def time_to_seconds(self ,time:str,Cancle:str):
        if not Cancle=="True":
            #its not cancle cmd
            try:                
    ...
    def post(self):
        ...
        time_sec=self.time_to_seconds(Time,Cancle)
        print(Demo_status)
        if Demo_status=="True" and Cancle=='False' :
            if time_sec>0:
                string={"STATUS":True,"Time":time_sec}
                print(f'[OPUTPUT]  (Temp) PC will {Action} in {time_sec} seconds\n')
                return jsonify(string)
            else:
                string={"STATUS":True,"Time":time_sec,"Error":"Wrong time input"}
            ...
            if time_sec>0:
                ...
                print(f'[OPUTPUT]  (Real) PC will {Action} in {time_sec} seconds\n')
                subprocess.run(["shutdown", "-a"],  shell=True, stdout=subprocess.PIPE, stdin=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                subprocess.run(["shutdown", "-s", "-t" ,f"{time_sec}"],  shell=True, stdout=subprocess.PIPE, stdin=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                return jsonify(string)
            else:
                string={"STATUS":True,"Time":time_sec,"Error":"Wrong time input\n"}
